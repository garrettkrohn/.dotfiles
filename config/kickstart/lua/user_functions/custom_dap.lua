...
local function start_java_debug()
  ...
  if not connected then
    vim.notify(
      string.format('❌ Cannot connect to %s:%d\nError: %s\n\nCheck DAP logs at: %s\n\nTry:\n1. Verify Java process is bound to 0.0.0.0 or 127.0.0.1\n2. Check firewall settings\n3. Run: telnet %s %d',
        host, debug_port, err or 'unknown', vim.fn.stdpath('cache') .. '/dap.log', host, debug_port),
      vim.log.levels.ERROR
    )
    return
  end
