return {
  ...
  -- Config is handled by require('config.lsp').setup() in init.lua
  config = false,
}

--[[ ORIGINAL CONFIG - Now handled by config/lsp/init.lua
return {
  -- Main LSP Configuration
  'neovim/nvim-lspconfig',
  event = { 'BufReadPre', 'BufNewFile', 'BufWritePre' },
  ...
  config = function()
    ...
    -- Note: mason-lspconfig will handle LSP server installation automatically
    -- based on the servers table above. Here we only need to specify additional
    -- tools like formatters and debug adapters.
