...
plugins=(git zsh-autosuggestions zsh-vi-mode nvm)
...
source $HOME/dotfiles/config/secret_aliases
...
# Node.js paths
export PATH="$HOME/.npm-global/bin:$HOME/.nvm/versions/node/v16.0.0/bin:$PATH"
...
export GOOSE_DBSTRING=user="postgres password=postgres dbname=db host=localhost port=3333 sslmode=disable"

# opencode
export PATH=/Users/gkrohn/.opencode/bin:$PATH
export PATH="$HOME/.local/bin:$PATH"

#gpg
export GPG_TTY=$(tty)

# Shortcut to insert clipboard contents into pass
pp() {
  if [[ -z $1 ]]; then
    echo "Usage: pp <pass_name>"
    return 1
  fi

  local pass_name="$1"

  # Read clipboard and insert into pass
  pbpaste | pass insert -m -f "$pass_name"

  echo "Password inserted into pass entry: $pass_name"
}
